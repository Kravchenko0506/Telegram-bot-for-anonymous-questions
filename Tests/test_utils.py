...
class TestCriticalInputValidation:
    ...
    def test_input_validator_validate_question(self):
        ...
        # Valid question
        is_valid, error = InputValidator.validate_question("How does this work?")
...
class TestCriticalMessageValidation:
    ...
    @pytest.mark.unit
    @pytest.mark.utils
    def test_callback_data_format_FIXED(self):
        """Test callback data format validation - FIXED VERSION."""
        ...
        # Invalid callback data - FIXED LOGIC
        invalid_callbacks = [
            "",           # Empty string
            "action",     # No colon
            ":123",       # Empty action
        ...
        for callback in invalid_callbacks:
            ...
            is_invalid = (
                ...
                (len(parts) >= 2 and parts[1] and not parts[1].isdigit())  # Invalid ID
            )
...
class TestValidatorIntegration:
    ...
    def test_content_moderator_integration(self):
        ...
        assert ContentModerator.is_likely_spam(normal_text, threshold=0.5) is False
        ...
        assert ContentModerator.is_likely_spam(obvious_spam, threshold=0.3) is True
